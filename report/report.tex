...
\begin{defi}
A \textit{volume} \V{} (resp. \textit{surface}) is a topological 3(resp. 2)-manifold embedded in $\mathbb{R}^3$.
...
\end{lemma}
\textit{Proof}:~~ \\
Let t be a volumic element (triangle). We use the \textit{cut locus} associated to t, i.e the the set of all points having more than one shortest path from t: we take as \Sys{} the non arborescent part of this cut locus, called the \textit{reduced} cut locus.
\fin
\begin{lemma}
...
\end{lemma}
Then we can give the pants optimization algorithm: compute a pants decomposition and optimize locally every boundary of a pant, until no improvement is possible.
...
\begin{defi}
The \textit{medial axis} (or \textit{skeleton}) of \S{} is the set of all points (not in \S{}) having more than one closest point on the object's boundary, i.e the centers of the open balls tangent to this boundary in at least two points.
\end{defi}
\begin{defi}
The \textit{local feature size} of a point on \S{} is its distance to the medial axis.
\end{defi}
...
%\fin
However, we can have intersections in this cut locus, and we are not sure to get 2-cuts: Marco Attene and
Jean-Marie Favreau suggested a method to avoid such non manifold cuts, but it is rather complicated and I didn't have the time to investigate it further. 
...
Then, we compute a max flow on N (using Ford Fulkerson algorithm, for example) and we use the min cut - max flow theorem to deduce a \textit{min \C{} s-t cut} (i.e, a min cut obtained by computing a max flow on \C) \MinC.
...
We assume in this subsection that every edge has capacity 1 (it is the case, for example if the capacity of an edge is equal to the area of the corresponding face).
\begin{theorem}
Let p be the shortest path from s to t (we assume that it is already computed with a BFS), \C{} a min s-t cut and C its total capacity. \\
The neighborhood algorithm (without the computation of the first path) has complexity O(pC$^2$).
\end{theorem}
\textit{Proof}:~~ \\
Throughout the algorithm, the neighborhood grows and at the end, its size is inferior to pC (the size of a complete cylinder with axis p), since the algorithm stop as soon as the min cut is found. \\ 
So we visit at most pC vertices for each BFS, and there are C such BFS.
\fin
...
I noticed an improvement of the time required by the NA (and CNA) when the quality (i.e, the radius-edge ratio of the tetrahedra) of the tetrahedrization increases, since the higher the quality, the smaller \DualAdj{} (and then the neighborhood is also small). \\
...
\hline
Complexity (edges + vertices) of the dual graph & Classic & NA & CNA \\ \hline
...
\hline
 & \multicolumn{ 3}{c|}{CNA} & \multicolumn{ 3}{c|}{NA} & \multicolumn{ 2}{c|}{Classic} \\ \hline
...
\end{center}
\end{figure}
\subsection{Computation of the first path}
Since I used a rather "big" min cut in the previous tests, the time required by the first BFS is rather negligible, but if the min cut is smaller it may be interesting to find a way to avoid this first BFS (for example if the min cut is such that the first 100 paths found gives a max flow, the first BFS takes a time equal to the half of the total time required by the CNA). I describe an algorithm to do this, with the SPA:
...
\begin{defi}
A loop is a continuous map \L{} : [0, 1] $\longrightarrow$ \S{} such that \L(0) = \L(1)
\end{defi}
TODO 
