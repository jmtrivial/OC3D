...
\end{itemize}
The second point is needed to avoid non-connex cuts [fig]. \\
...
However, the final weight of \Sys{} and the complexity of these algorithms depends crucially on the order we choose cuts to optimize. [fig] \\
...
However for the MPA this choice is more problematic since there are too many 2-cuts (every system of 2-cuts contains g cuts). \\
The most "obvious" choice is to iteratively select the shortest 2-cut, while possible, but this may not be the best choice [fig]. \\
...
\fin
In the case of tetrahedra, this result is false since we can have an arbitrary number of tetrahedra adjacent to a single tetrahedra (however, the algorithm may perform very well on tetrahedra, see \ref{time}. [fig] \\
...
\begin{theorem}
Let r $\geqslant$ 3.\\
Almost all r-regular graph of order n has diameter K log(n), with K constant (informally, the probability of selecting such a graph tends to 1 as n increases).
\end{theorem}
...
\centering \includegraphics[height=2cm]{sectionnoq.eps}
\end{minipage}
\hfill
\begin{minipage}{0.3\linewidth}
\centering \includegraphics[height=2cm]{sectionq.eps}
\end{minipage}
\caption{Left: The Koch snowflake, a surface with finite area but with infinite perimeter. 
Right: The optimal 1-cut (in blue) is different from the optimal 2-cut (in red) of this torus. }
\label{Koch}
