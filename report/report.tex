...
The strict p-neighborhood \Ns{p} is the set of all paths p' such that d(p, p') = 1. \\
...
\begin{theoreme}
Assume we are computing a max flow in \V, using neighborhood algorithm and that we already found n (n $\leqslant$ 1) augmenting paths \Path = $\lbrace p_1, \ldots, p_n \rbrace$. \\
...
\end{table}
I didn't have the time to test this algorithm on voxels, but for non degenerated volume, the first neighborhood will be roughly four times the length of the first path found. \\
For that example, it means that the first neighborhood is likely be about 400 edges instead of 2452 for tetrahedra, eight times less. \\
Then I expect the neighborhood algorithm to be much more efficient with voxels.
