...
Although they have similarities, the two problems above are not identicals and especially, minimizing the area of a surface is not equivalent to minimizing its perimeter. [Cylinder with a "star"].\\
...
In the SPA, one may wonder whether we keep the pant structure:
\begin{theoreme}
(\Sys - \C) $\cup$ \MinC{} is a system of 2-cuts.
\end{theoreme}
A result of "stability" with respect from the initial 2-cut system follows:
\begin{theoreme}
Assume that there is only one 2-cut with weight equals to the weight of \C$_i$, for all i. \\
Then the resulting cutting of the whole SPA doesn't depend on the initial cutting.
\end{theoreme}
\begin{rem}
It may be interesting, if two min 2-cuts have the same weight, to choose the "best", dans un sens à préciser, this can be done using a min cost max flow algorithm.
\end{rem}
...
Like auto cut case, we build N from \Dual:
\begin{itemize}
